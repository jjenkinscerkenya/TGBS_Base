...
def calc_nirv(image: ee.Image):
    ...
    return image.select("SR_B5").multiply(calc_ndvi(image)).rename("NIRv")


# Multi-index helpers
def calc_veg_indices(image: ee.Image):
    """Add the original vegetation and water indices discussed for the baseline Landsat-8 workflow."""
...
def calc_tgbs_indices(image: ee.Image):
    """Add the full TGBS-oriented Landsat-8 index set for vegetation, moisture, disturbance, and biomass-proxy analysis."""
    return image.addBands(
        [
            calc_ndvi(image),
            calc_evi(image),
...
def select_base_l8_bands(image: ee.Image):
    """Select the core Landsat-8 reflectance bands commonly used to derive the project indices."""
